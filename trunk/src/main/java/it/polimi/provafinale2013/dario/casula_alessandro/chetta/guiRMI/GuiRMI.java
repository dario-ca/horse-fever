...
public class GuiRMI implements GuiHorseFeverRMI{
	...
	private final String backCards = "img/Personaggi/retro.jpg";
	...
	private final String registryAddress = "127.0.0.1";
	
	//variabile booleana che mi dice se devo mostrare il fronte delle carte azione o no
	private Boolean areActionCardsVisible;
	//IMMAGINI & LABELS
	...
	public GuiRMI(){
		...
		try {
        	...
        	// esporto lo stub del client (cosi' puo' essere inviato al server)
        	UnicastRemoteObject.exportObject(this, 0);
        	// prendo un riferimento al registry
            Registry registry = LocateRegistry.getRegistry(registryAddress);
            // cerco lo stub del server nel registry
            modelController = (ControllerHorseFeverRMI) registry.lookup("HFServer");
        } catch (Exception e) {
            new RemoteExceptionAlertFrame("errore nel recuperare il server"+e.getMessage());
        }
    ...
    public void startTakePlayers(String error){
    	new PlayerManagerPopUpRMI(modelController, this, error);
    }
    
    public void startBetsManagerPopUp(String error, Boolean isMandatory){
    	new BetsManagerPopUpRMI(modelController, this, isMandatory, error);
    }
    
    public void startActionCardManagerPopUp(){
    	new ActionCardsManagerPopUpRMI(modelController, this);
    }
    
    public void startComunicateLoosersPopUp(ArrayList<Integer> loosersPlayerIndexes){
    	new ComunicateLoosersPopUpRMI(loosersPlayerIndexes, modelController);
    }
   ...
   private void createJTabbedPane()
   {
    ...
    private JPanel currentPlayerControl(){
    	...
    	//informo l'utente di asettare la connessione di almeno un altro utente
    	//currentPlayerInfoLabel.setText("Attendere la connessione di altri utenti ")
    	currentPlayerInfoLabel.setText("Attendere ulteriori utenti");
    	panel.add(currentPlayerInfoLabel);
    	panel.setPreferredSize(new Dimension(0, 35));
    	controlButton = new JButton("Inizia");
    	controlButton.setEnabled(false);
    	controlButton.addActionListener(new PeekNewCardHandlerRMI(modelController));
    ...
    private void createMyActionCardsPanel(){
		...
		for(int i=0; i<2; i++)
		{
			actionCardLabels.add(new JLabel(new ImageIcon(backCards)));
    ...
    private void createMyCardsPanel(){
		...
		//carico delle immagini del retro delle carte in attesa di ricevere le mie
		characterCardLabel.setIcon(new ImageIcon(backCards));
		stableCardLabel.setIcon(new ImageIcon(backCards));
		//metto le immagini
    ...
    private void createBottomPanel(){
		...
		//riempio bottom
		for(int i=0; i<ControllerHorseFeverRMI.numberOfStables; i++)
			{
    ...
    private JPanel createOddsPanel(){
    	...
    	for(int i=0; i<ControllerHorseFeverRMI.numberOfStables; i++)
    	{
    		...
    		for(int j=0; j<ControllerHorseFeverRMI.numberOfStables; j++)
    			{
    ...
    private void createTabPanel(){
		...
		tabPanel.setLayout(new GridLayout(numberOfRows,ControllerHorseFeverRMI.numberOfStables));
		tabPanel.setPreferredSize(new Dimension(447, 536));
    ...
    private void updatePlayerActionCards(){
		//chiedo al controllore le carte azione del currentplayer
    	try{
	    	List<ActionCard> actionCards =  modelController.getPlayerActionCards(this);
    		...
    		new RemoteExceptionAlertFrame("errore nell'aggiornamento delle proprie carte azione "+e.getMessage());
    	}
    }
    ...
    private void updatePlayerCards(){
		...
		try {
			characterCard = modelController.getPlayerCharacterCard(this).getCardFrontImage();

		characterCardLabel.setIcon(characterCard);
		//chiedo al controller la carta scuderia del currentplayer
		stableCard=modelController.getPlayerStableCard(this).getCardFrontImage();
		stableCardLabel.setIcon(stableCard);
		} catch (RemoteException e) {
			new RemoteExceptionAlertFrame("errore nella lettura delle carte personali"+e.getLocalizedMessage());
		}
    }
    ...
    private void updateBets(){
		...
		try {
			numberOfPlayers = modelController.getNumberOfPlayers();
		...
		{
			List<Bet> betOfThisPlayer;
		
				betOfThisPlayer = modelController.getBetsOfThePlayer(k);
				...
				new RemoteExceptionAlertFrame("errore nell'aggiornamento delle scommesse");
			}
		}
    ...
    private void updateCurrentPlayerInfo(){
    	...
    	try{
	    	String currentPlayerInfo = "Giocatore corrente: "+modelController.getPlayerName(this);
	    	currentPlayerInfo += " | Danari: "+modelController.getPlayerDanariAmount(this);
	    	currentPlayerInfo += " | PV: "+modelController.getPlayerPV(this);
	    	//aggiorno la label
	    	currentPlayerInfoLabel.setText(currentPlayerInfo);}
    	catch(Exception e){
    		new RemoteExceptionAlertFrame("Errore nell'estrazione delle informazioni utente"+e.getMessage()); 
    	}
    }
    ...
    private void updateOddsBlackBoard(){
    	...
    	{//chiedo l'array per questa quotazione
    		List<StableCard> stablesOnThisOdds = modelController.getStablesCardAt(i);
			...
			new RemoteExceptionAlertFrame("errore nell'aggironamento della obb");
		}
    }
    ...
    private void updateRanking(){
    	...
    	List<StableCard> ranking;
		
			ranking = modelController.getRanking();
    	...
    	for(int i=0; i<ranking.size(); i++)
			...
			new RemoteExceptionAlertFrame("Errore nell'aggiornamento della ranking");
		}
    }
    ...
    private void updateTab(){
    	...
    	List<Integer> allPawnPosition;
		
			allPawnPosition = modelController.getAllPawnPosition();
    	...
    	for(int i=0; i<ControllerHorseFeverRMI.numberOfStables; i++)
			...
			new RemoteExceptionAlertFrame("Errore nell'aggiornamento del tabellone");
		}
    }
    ...
    private void updateOtherPlayers(){
    	...
    	for(int i=0,j=0; i<numberOfPlayers; i++,j+=2){
    		...
    		String currentPlayerName = modelController.getPlayerName(this);
			...
			new RemoteExceptionAlertFrame("Errore nell'aggiornamento della tabella degli altri giocatori");
		}
    }
    ...
    private void updateLanesActionCards(){
    	...
    	for(int i=0; i<ControllerHorseFeverRMI.numberOfStables; i++){
    		...
    		List<ActionCard> actionCardsOnThisLane;
			
				actionCardsOnThisLane = modelController.getActionCardsOnLane(i);
    		...
    		for(int j=0; j<actionCardsOnThisLane.size(); j++)
    			...
    			new RemoteExceptionAlertFrame("Errore nell'aggiornamento della carte azione sulle lanes");
    		}
    	}
    }
