...
public class BetsManagerPopUp implements PopUp{
	...
	public BetsManagerPopUp(HorseFeverController modelController, Boolean isMandatory, String error){
	...
	public BetsManagerPopUp(HorseFeverController modelController, Boolean isMandatory){
		this.modelController = modelController;
		this.isMandatory = isMandatory;
		this.error = "";
		setup();
	...
	private void setup(){
		...
		betsManagerFrame.setIconImage(Toolkit.getDefaultToolkit().getImage(GUI.icona));
		betsManagerFrame.setLocationRelativeTo(null);
		betsManagerFrame.setSize(400, 320);
		betsManagerFrame.setResizable(false);
		...
		AmountButtonsHandler amountButtonHandler = new AmountButtonsHandler(this);
		...
		StableChoiceButtonHandler stableChoiceButtonHandler = new StableChoiceButtonHandler(this);
		...
		BetTypeButtonHandler betTypeButtonHandler = new BetTypeButtonHandler(this);
		JPanel betTypeChoice = new JPanel(new GridLayout(3,1));
		typeChosen = new JLabel();
		typeChosen.setIcon(new ImageIcon(GUI.betToWinLogo));
		...
		ManageBetButtonHandler manageBetButtonHandler = new ManageBetButtonHandler(this);
	...
	public void updateBetType(Boolean isToWin){
		if(isToWin)
		{
			...
			{
			typeChosen.setIcon(new ImageIcon(GUI.betToShowLogo));
			}
